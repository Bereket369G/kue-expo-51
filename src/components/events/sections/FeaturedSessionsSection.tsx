  ...
  return <section className="mb-12 relative">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="relative p-1 w-48 bg-gray-800 mx-[38px] rounded-full">
          <div className={`absolute top-1 transition-transform duration-200 ease-in-out ${activeDay === 'day1' ? 'translate-x-0' : 'translate-x-full'} w-24 h-8 bg-[#1A1F7C] rounded-full`} />
          <div className="relative flex">
            <button onClick={() => setActiveDay('day1')} className={`flex-1 h-8 text-sm font-medium transition-colors duration-200 z-10 ${activeDay === 'day1' ? 'text-white' : 'text-gray-600'}`}>
              Day 1
            </button>
            <button onClick={() => setActiveDay('day2')} className={`flex-1 h-8 text-sm font-medium transition-colors duration-200 z-10 ${activeDay === 'day2' ? 'text-white' : 'text-gray-600'}`}>
              Day 2
            </button>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="flex overflow-x-auto gap-4 pb-4 hide-scrollbar snap-x snap-mandatory">
        {currentSessions.map((session, index) => <EventCard key={index} type={session.type} title={session.title} time={session.time} location={session.location} startsIn={session.startsIn} accessRequired={session.accessRequired} typeStyle={session.typeStyle} onClick={() => navigate(session.route)} />)}
      </div>
      {showArrow && <button onClick={onScroll} className="absolute right-0 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-[#1A1F7C] rounded-full flex items-center justify-center text-white shadow-lg z-10 hover:bg-[#2A307C] transition-colors">
